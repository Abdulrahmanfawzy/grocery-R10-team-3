...
const ProductCard = ({ id, category, title, price, oldPrice, rating, image, brand = "Mr food", inStock = true }: ProductCardProps) => {
    ...
    <Card className="h-full overflow-hidden border-none shadow-md hover:shadow-2xl transition-all duration-300 relative group">
      <CardContent className="p-3 flex flex-col h-full">
        <div className="relative aspect-square mb-2 bg-gray-50 rounded-lg overflow-hidden flex items-center justify-center p-0">
          {image ? (
            <img src={image} alt={title} className="max-h-full max-w-full object-contain group-hover:scale-110 transition-transform" />
          ) : (
        ...
        <p className="text-[10px] text-gray-400 mb-3">By <span className="text-blue-900">{brand}</span></p>
