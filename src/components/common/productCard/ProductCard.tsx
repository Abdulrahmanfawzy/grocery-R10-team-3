        ...
        <div className="w-full bg-white border border-[#DAD8D8] rounded-[8px] p-[16px] relative group">
                ...
                {product.badges.map((badge, index) => (
                    <span
                        ...
                        className="
                            ...
                            whitespace-nowrap
                        "
                        style={{background: "linear-gradient(0deg, rgba(1, 65, 98, 0.5) -28.12%, rgba(1, 65, 98, 0.8) 30.45%, #014162 87.19%)"}}
                    >
                        {badge}
                    </span>
                ))}
            ...
            <div className="flex items-center justify-center gap-[4px] mb-[20px]">
                <div className="flex text-[#FDC040]">
                    {[1, 2, 3, 4, 5].map((star) => (
                        <span key={star} className={`text-[24px] ${star > Math.floor(product.rating) ? 'text-[#DAD8D8]' : ''}`}>
                            ★
                        </span>
                    ))}
                </div>
                <span className="text-[12px] text-[#BCB8B1]">
                    ({product.rating}/5)
                </span>
            </div>
